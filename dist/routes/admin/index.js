...
router.post("/players/update/:id", auth_1.isStaff, (req, res, next) => {
    // invoke the single file upload middleware attached earlier in app
    if (req.fileUpload) {
        req.fileUpload(req, res, (err) => {
            if (err) {
...
router.post("/tickets/range", auth_1.isAdmin, ticketController_1.setTicketRange);
router.post("/tickets/resync", auth_1.isAdmin, ticketController_1.resyncTicketCounter);
// Age bracket routes
router.get("/age-brackets", auth_1.isAdmin, ageBracketController_1.getAgeBrackets);
router.post("/age-brackets", auth_1.isAdmin, ageBracketController_1.addAgeBracket);
router.put("/age-brackets/:id", auth_1.isAdmin, ageBracketController_1.updateAgeBracket);
router.delete("/age-brackets/:id", auth_1.isAdmin, ageBracketController_1.deleteAgeBracket);
// Raffle routes
router.get("/raffle", auth_1.isAdmin, raffleController_1.getRaffleInterface);
router.post("/raffle/draw", auth_1.isAdmin, raffleController_1.drawRaffleWinner);
// Event Locations routes
router.get("/event-locations", auth_1.isAdmin, dashboardController_1.getSettings);
router.post("/event-locations/location/add", auth_1.isAdmin, dashboardController_1.addEventLocation);
router.post("/event-locations/location/delete/:id", auth_1.isAdmin, dashboardController_1.deleteEventLocation);
// Content management routes
router.get("/content/:section", auth_1.isAdmin, contentController_1.getEditableContent);
router.post("/content/update", auth_1.isAdmin, contentController_1.updateContent);
// Sales tracking routes
router.get("/sales-reports", auth_1.isAdmin, salesReportsController_1.getSalesReports);
router.get("/api/sales-tracking", auth_1.isAdmin, salesReportsController_1.getSalesTrackingData);
router.get("/api/sales-yearly", auth_1.isAdmin, salesReportsController_1.getYearlySalesData);
router.get("/api/sales-yearly-download", auth_1.isAdmin, salesReportsController_1.downloadYearlySalesReport);
