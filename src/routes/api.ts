...
import {
  createCompetition,
  getCompetitions,
} from "../controllers/api/competitionController";
import {
...
router.post("/events/register", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.log(
        `Tickets needed: ${totalTicketsNeeded} (${kicksRequested} kicks × ${ticketsPerKick} tickets/kick)`
      );
...
router.post("/notifications/raffle-win", async (req, res) => {
  ...
  try {
    ...
    if (customPrize && customPrize.trim()) {
      message = `Great news ${
        player.name
      }! Your ticket #${ticketNumber} won daily raffle #${raffleDrawNumber} drawn on ${formattedDate}. You won: ${customPrize.trim()}! Please contact staff to claim your prize!`;
    } else {
