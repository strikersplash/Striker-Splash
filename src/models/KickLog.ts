...
export interface IKickLog {
  id: number;
  player_id: number;
  staff_id: number;
  competition_type: "match" | "solo";
  match_id?: number;
  solo_competition_id?: number;
  goals: number;
  kicks_used: number;
  location?: string;
  team_id?: number;
  consecutive_kicks?: number;
  notes?: string;
  created_at: Date;
...
export class KickLog {
  static async create(kickData: Partial<IKickLog>): Promise<IKickLog> {
  ...
  static async findById(id: number): Promise<IKickLog | null> {
    const client = await pool.connect();
    try {
      const query = `
        SELECT kl.*, 
  ...
  static async findByMatch(matchId: number): Promise<IKickLog[]> {
    const client = await pool.connect();
    try {
      const query = `
        SELECT kl.*, 
  ...
  static async findBySoloCompetition(
    soloCompetitionId: number
  ): Promise<IKickLog[]> {
    const client = await pool.connect();
    try {
      const query = `
  ...
  static async getMatchStats(matchId: number): Promise<any> {
    const client = await pool.connect();
    try {
      const query = `
        SELECT 
  ...
  static async getSoloCompetitionStats(
    soloCompetitionId: number
  ): Promise<any[]> {
    const client = await pool.connect();
    try {
  ...
  static async getActivity(startDate: Date, endDate: Date): Promise<any[]> {
    const client = await pool.connect();
    try {
      const query = `
        SELECT 
