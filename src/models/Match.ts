...
export interface IMatch {
  id: number;
  name: string;
  match_type: "3v3" | "5v5" | "10v10" | "11v11";
  team_a_id: number;
  team_b_id: number;
...
export class Match {
  static async create(matchData: Partial<IMatch>): Promise<IMatch> {
  ...
  static async findAll(limit: number = 50): Promise<IMatch[]> {
    const client = await pool.connect();
    try {
      const query = `
        SELECT m.*, 
  ...
  static async findByStatus(status: string): Promise<IMatch[]> {
    const client = await pool.connect();
    try {
      ...
      if (match_participants_exists) {
        ...
        const goalsCheck = await client.query(checkGoalsQuery);
  ...
  static async getRecent(limit: number = 10): Promise<IMatch[]> {
    const client = await pool.connect();
    try {
      // First check if match_participants table exists
      const checkTableQuery = `
  ...
  static async addParticipant(
    ...
    try {
      ...
      await client.query(query, [matchId, teamId, playerId]);
      return true;
    } catch (error) {
  ...
  static async getParticipants(matchId: number): Promise<any[]> {
    const client = await pool.connect();
    try {
      const query = `
  ...
  static async updateParticipantKicks(
    ...
    try {
      ...
      await client.query(query, [matchId, playerId, kicksUsed]);
      return true;
    } catch (error) {
