...
export class SoloCompetition {
  static async create(
    competitionData: Partial<ISoloCompetition>
  ): Promise<ISoloCompetition> {
  ...
  static async findById(id: number): Promise<ISoloCompetition | null> {
    const client = await pool.connect();
    try {
      const query = `
        SELECT sc.*, 
  ...
  static async findAll(limit: number = 50): Promise<ISoloCompetition[]> {
    const client = await pool.connect();
    try {
      const query = `
        SELECT sc.*, 
  ...
  static async findByStatus(status: string): Promise<ISoloCompetition[]> {
    const client = await pool.connect();
    try {
      const query = `
        SELECT 
  ...
  static async getRecent(limit: number = 10): Promise<ISoloCompetition[]> {
    const client = await pool.connect();
    try {
      const query = `
        SELECT 
  ...
  static async updateStatus(
    ...
    status: string,
    staffId?: number
  ): Promise<ISoloCompetition | null> {
    const client = await pool.connect();
    try {
      let query =
        "UPDATE solo_competitions SET status = $1, updated_at = CURRENT_TIMESTAMP";
  ...
  static async addParticipant(
    ...
    try {
      ...
      const { max_participants, current_participants } =
  ...
  static async getParticipants(competitionId: number): Promise<any[]> {
    const client = await pool.connect();
    try {
      const query = `
  ...
  static async updateParticipantKicks(
    competitionId: number,
    playerId: number,
    kicksUsed: number
  ): Promise<boolean> {
  ...
  static async getLeaderboard(competitionId: number): Promise<any[]> {
    const client = await pool.connect();
    try {
      const query = `
