...
class QueueTicket {
  ...
  static async create(data: { player_id: number, competition_type: string, official?: boolean }): Promise<IQueueTicket | null> {
    try {
      const { player_id, competition_type, official = true } = data;
      
      // Get next ticket number
      const ticketNumber = await QueueTicket.getNextTicketNumber();
      
      const result = await pool.query(
        'INSERT INTO queue_tickets (ticket_number, player_id, status, competition_type, official) VALUES ($1, $2, $3, $4, $5) RETURNING *',
        [ticketNumber, player_id, 'in-queue', competition_type || 'accuracy', official]
      );
  ...
  static async updateStatus(id: number, status: 'in-queue' | 'played' | 'expired'): Promise<IQueueTicket | null> {
    try {
      let query = '';
      
      if (status === 'played') {
        query = 'UPDATE queue_tickets SET status = $1, played_at = NOW() WHERE id = $2 RETURNING *';
      } else if (status === 'expired') {
        query = 'UPDATE queue_tickets SET status = $1, expired_at = NOW() WHERE id = $2 RETURNING *';
      } else {
        query = 'UPDATE queue_tickets SET status = $1 WHERE id = $2 RETURNING *';
      }
