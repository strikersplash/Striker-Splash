...
class Staff {
  ...
  static async findOne(criteria: { username?: string }): Promise<IStaff | null> {
    try {
      if (criteria.username) {
        const result = await pool.query('SELECT * FROM staff WHERE username = $1', [criteria.username]);
        return result.rows[0] || null;
      }
      return null;
    } catch (error) {
  ...
  // Create new staff
  static async create(staffData: { username: string; password: string; name: string; role: string }): Promise<IStaff | null> {
    try {
  ...
  static async find(): Promise<IStaff[]> {
    try {
      const result = await pool.query('SELECT id, username, name, role, created_at, updated_at FROM staff');
      return result.rows;
    } catch (error) {
