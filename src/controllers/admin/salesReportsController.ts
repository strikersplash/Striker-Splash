...
export const getSalesReports = async (
  ...
  try {
    ...
    const currentDateResult = await executeQuery(
    ...
    switch (period) {
      case "week":
        ...
        const weekStartResult = await executeQuery(`
      ...
      case "month":
        ...
        const monthStartResult = await executeQuery(`
    ...
    console.log(
      `Sales report period: ${period}, from ${startDate} to ${endDate}`
    );
    ...
    // Calculate totals
    const totals = {
      totalCustomers: salesData.reduce(
        (sum: number, row: any) => sum + parseInt(row.customers_served),
        0
      ),
      totalTransactions: salesData.reduce(
        (sum: number, row: any) => sum + parseInt(row.total_transactions),
        0
      ),
      ...
      totalKicks: salesData.reduce(
        (sum: number, row: any) => sum + parseInt(row.total_kicks_sold),
        0
      ),
    };
...
export const getSalesTrackingData = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    const totals = {
      today: {
        customers: salesData.reduce(
          (sum: number, row: any) => sum + parseInt(row.customers_today),
          0
        ),
        revenue: salesData.reduce(
          (sum: number, row: any) => sum + parseFloat(row.revenue_today),
          0
        ),
      },
      week: {
        customers: salesData.reduce(
          (sum: number, row: any) => sum + parseInt(row.customers_week),
          0
        ),
        revenue: salesData.reduce(
          (sum: number, row: any) => sum + parseFloat(row.revenue_week),
          0
        ),
      },
      month: {
        customers: salesData.reduce(
          (sum: number, row: any) => sum + parseInt(row.customers_month),
          0
        ),
        revenue: salesData.reduce(
          (sum: number, row: any) => sum + parseFloat(row.revenue_month),
          0
        ),
      },
    };
...
export const getYearlySalesData = async (
  ...
  try {
    ...
    const salesResult = await executeQuery(salesQuery, [targetYear]);
    const salesData = salesResult.rows;
    ...
    const totals = {
      year: {
        customers: salesData.reduce(
          (sum, row) => sum + parseInt(row.customers_year),
          0
        ),
        revenue: salesData.reduce(
          (sum, row) => sum + parseFloat(row.revenue_year),
          0
        ),
        kicks: salesData.reduce(
          (sum, row) => sum + parseInt(row.kicks_year),
          0
        ),
        transactions: salesData.reduce(
          (sum, row) => sum + parseInt(row.transactions_year),
          0
        ),
      },
      monthly: {
        jan: salesData.reduce(
          (sum, row) => sum + parseFloat(row.jan_revenue),
          0
        ),
        feb: salesData.reduce(
          (sum, row) => sum + parseFloat(row.feb_revenue),
          0
        ),
        mar: salesData.reduce(
          (sum, row) => sum + parseFloat(row.mar_revenue),
          0
        ),
        apr: salesData.reduce(
          (sum, row) => sum + parseFloat(row.apr_revenue),
          0
        ),
        may: salesData.reduce(
          (sum, row) => sum + parseFloat(row.may_revenue),
          0
        ),
        jun: salesData.reduce(
          (sum, row) => sum + parseFloat(row.jun_revenue),
          0
        ),
        jul: salesData.reduce(
          (sum, row) => sum + parseFloat(row.jul_revenue),
          0
        ),
        aug: salesData.reduce(
          (sum, row) => sum + parseFloat(row.aug_revenue),
          0
        ),
        sep: salesData.reduce(
          (sum, row) => sum + parseFloat(row.sep_revenue),
          0
        ),
        oct: salesData.reduce(
          (sum, row) => sum + parseFloat(row.oct_revenue),
          0
        ),
        nov: salesData.reduce(
          (sum, row) => sum + parseFloat(row.nov_revenue),
          0
        ),
        dec: salesData.reduce(
          (sum, row) => sum + parseFloat(row.dec_revenue),
          0
        ),
      },
    };
...
export const downloadYearlySalesReport = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    const totalKicks = salesData.reduce(
      (sum, row) => sum + parseInt(row.kicks_year),
      0
    );
