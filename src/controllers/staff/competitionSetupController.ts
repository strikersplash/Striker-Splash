...
interface CompetitionData {
  ...
  participants?: number[];
  teams?: number[];
  selected_players?: { [key: string]: number[] }; // Maps team ID to array of selected player IDs
}
      ...
      if (
        ...
        for (const participantId of competitionData.participants) {
          ...
          const insertResult = await client.query(
            `INSERT INTO competition_players (competition_id, player_id)
        ...
        for (const teamId of competitionData.teams) {
          ...
          if (
            ...
            for (const playerId of selectedPlayers) {
              await client.query(
                `INSERT INTO custom_competition_active_players 
                 (competition_id, team_id, player_id, status, selected_at)
                 VALUES ($1, $2, $3, 'active', NOW())`,
                [competition.id, teamId, playerId]
              );
            }
          }
        }
...
export const getParticipantsWithLoggedGoals = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
...
export const getActiveTeamPlayers = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    const activePlayersQuery = await pool.query(
      `SELECT cp.competition_id, cp.team_id, cp.player_id, cp.status,
              p.name as player_name
       FROM custom_competition_active_players cp
       JOIN players p ON cp.player_id = p.id
       WHERE cp.competition_id = $1 AND cp.status = 'active'
       ORDER BY cp.team_id, p.name`,
      [competitionId]
    );
...
export const saveActiveTeamPlayers = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    ...
    const teamQuery = await pool.query(
      `SELECT * FROM competition_teams WHERE competition_id = $1 AND team_id = $2`,
    ...
    await pool.query(
      `UPDATE custom_competition_active_players 
       SET status = 'inactive' 
    ...
    for (const playerId of activePlayers) {
      ...
      const playerQuery = await pool.query(
        `SELECT * FROM custom_competition_active_players 
         WHERE competition_id = $1 AND team_id = $2 AND player_id = $3`,
        [competitionId, teamId, playerId]
      );
